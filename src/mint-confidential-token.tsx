...
export const MintConfidentialToken: React.FC<MintConfidentialTokenProps> =( {instance, fileCid, contentEncryptionKey} ) => {
  const { 
    data: hash,
    error,   
    isPending, 
  ...
  const [missingCidOrKeyError, setMissingCidOrKeyError] = React.useState<boolean>(false);
  ...
  async function submit(e: React.FormEvent<HTMLFormElement>) { 
    ...
    const splitedEncryptionKey = splitBigIntToUint64Array(contentEncryptionKey);
    // console.log('splited clearData', splitedClearData);
    const encryptedData = splitedEncryptionKey.map(n => instance?.encrypt64(n)).map(toHexString);
    ...
    try {
      writeContract({
        ...
        args: [address, decodeBase58(fileCid.slice(4)), BigInt(1), encryptedData, '0x'],
      })
    } catch (error) {
      console.error(error);
