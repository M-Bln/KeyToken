...
export const UploadToLighthouse : React.FC<UploadToLighthouseProps> = ({encryptedFile, setFileCid, fileCid}) => {
        ...
        function createFileList(file0: File, file1: File) {
          ...
          const uploadEncryptedFile = async() =>{
            ...
            if (lighthouseApiKey) {
                ...
                const blob2 = new Blob([encryptedFile, encryptedFile], { type: 'application/octet-stream' });
                ...
                const output = await lighthouse.upload(fileList, lighthouseApiKey, true, undefined);
                console.log('File Status:', output);
                console.log('Visit 0 at https://gateway.lighthouse.storage/ipfs/' + output.data[0].Hash);
                console.log('Visit 1 at https://gateway.lighthouse.storage/ipfs/' + output.data[1].Hash);
    ...
    return(
        <div>
            ...
            You can upload the encrypted file with lighthouse. You then rely on a third party rather than having to run your own IPFS node. First you need to optain a Lighthouse API key ...., it only require to identify with metamask wallet.
